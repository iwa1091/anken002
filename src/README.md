...
## 使用技術
本プロジェクトの構築には、以下の技術スタックが使用されています。

- Docker/Docker Compose: コンテナベースの開発環境を構築するための基盤技術です。

- Laravel: PHP製のWebアプリケーションフレームワーク。

- Nginx: Webサーバーとして、PHP-FPMと連携して動作します。

- PHP-FPM: PHPスクリプトを実行するプロセス管理ソフトウェアです。

- MySQL: データベース管理システムとして、アプリケーションのデータを永続化します。

- phpMyAdmin: MySQLデータベースをブラウザから管理するためのツールです。

- MailHog: 開発中にメールの送信テストを行うための、シンプルで便利なSMTPサーバーです。
...
## 管理者・一般ユーザーのログイン情報
本アプリケーションは、初期セットアップ時に以下のテストユーザーが自動的に作成されます。
